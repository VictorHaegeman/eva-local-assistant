...
class Settings:
    ...
    eva_screen_training_enabled: bool = _env_bool("EVA_SCREEN_TRAINING_ENABLED", True)
    eva_screen_training_max_rounds: int = _env_int("EVA_SCREEN_TRAINING_MAX_ROUNDS", 14)
    eva_browser_extension_reasoning_timeout_seconds: float = _env_float(
        "EVA_BROWSER_EXTENSION_REASONING_TIMEOUT_SECONDS",
        75.0,
    )
    ...
    eva_curiosity_enabled: bool = _env_bool("EVA_CURIOSITY_ENABLED", False)
    eva_curiosity_interval_minutes: int = _env_int("EVA_CURIOSITY_INTERVAL_MINUTES", 180)
    eva_curiosity_max_items_per_run: int = _env_int("EVA_CURIOSITY_MAX_ITEMS_PER_RUN", 5)
    eva_curiosity_min_score: int = _env_int("EVA_CURIOSITY_MIN_SCORE", 10)
    eva_curiosity_wikipedia_pages: int = _env_int("EVA_CURIOSITY_WIKIPEDIA_PAGES", 2)
    eva_curiosity_rebuild_embeddings: bool = _env_bool("EVA_CURIOSITY_REBUILD_EMBEDDINGS", False)
    ...
    eva_job_runner_poll_seconds: int = _env_int("EVA_JOB_RUNNER_POLL_SECONDS", 5)
    eva_job_runner_max_attempts: int = _env_int("EVA_JOB_RUNNER_MAX_ATTEMPTS", 2)
    eva_job_checkpoint_every: int = _env_int("EVA_JOB_CHECKPOINT_EVERY", 12)
